...
# Load Data
temp_data = pd.read_csv(DATA / 'Temperature_Data_Merredin.csv')
rainfall_data = pd.read_csv(DATA / 'Rainfall_Data_Merredin.csv')
soil_data = pd.read_csv(DATA / 'Soil_Parameters.csv')
crop_data = pd.read_csv(DATA / 'Crop_Parameters_Wheatbelt.csv')
irrigation_data = pd.read_csv(DATA / 'Irrigation_Parameters.csv')

# Filter soil data for Wheatbelt (Merredin)
soil_data = soil_data[soil_data['Region'] == 'Merredin']
...
# Crop_Parameters_Wheatbelt.csv is not one of this region's soil types in
...
# Features and target
features = ['Optimal Temperature (°C)', 'Total Rainfall (mm)', 'Soil pH', 'Nitrogen (N%)', 'Phosphorus (P%)', 'Potassium (K%)']
target = 'Crop'
